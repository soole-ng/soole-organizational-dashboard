...
declare class MapWorker {
	...
	syncRTLPluginState(mapId: number, state: ActorMessages["syncRTLPluginState"]["params"], callback: ActorMessages["syncRTLPluginState"]["callback"]): Promise<void>;
...
declare class SymbolStyleLayer extends StyleLayer {
	...
	getValueAndResolveTokens<T extends keyof LayoutProps$2>(name: T, feature: Feature, canonical: CanonicalTileID, availableImages: ImageId[]): string;
	getAppearanceValueAndResolveTokens<T extends keyof AppearanceProps>(appearance: SymbolAppearance, name: T, feature: Feature, canonical: CanonicalTileID, availableImages: ImageId[]): string | LayoutProps$2[T];
...
declare class SymbolPropertyBinderUBO {
	 ...
	 */
	populateUBO(feature: Feature, vtFeatureIndex: number, canonical: CanonicalTileID, availableImages: ImageId[], brightness?: number | null, formattedSection?: FormattedSection): number;
	/**
...
declare class ProgramPrecompiler {
	...
	constructor();
	needsBuild(): boolean;
	buildQueue(layers: TypedStyleLayer[], parameters: EvaluationParameters, style: Style$1): void;
	_executeBatch(deadline: IdleDeadline, painter: Painter, style: Style$1): void;
	processQueue(painter: Painter, style: Style$1): void;
	reset(): void;
}
...
export declare class RasterTileSource<T = "raster"> extends Evented<SourceEvents> implements ISource<T> {
	...
	hasTile(tileID: OverscaledTileID): boolean;
	loadTile(tile: Tile, callback: Callback<undefined>): void;
	loadTileWithProvider(tile: Tile, provider: TileProvider<ArrayBuffer | ImageBitmap>, request: RequestParameters, controller: AbortController, callback: Callback<undefined>): Promise<void>;
	abortTile(tile: Tile, callback?: Callback<undefined>): void;
	unloadTile(tile: Tile, callback?: Callback<undefined>): void;
	hasTransition(): boolean;
...
declare class IndoorManager extends Evented<IndoorEvents> {
	...
	constructor(style: Style$1, styleLoaded?: boolean);
	destroy(): void;
...
declare class FillBucket implements Bucket {
	 ...
	 * @private
	 */
	addGeometry(polygons: Point[][][], bufferData: FillBufferData, elevationParams?: ElevationParams, elevatedStructures?: ElevatedStructures): [
		number,
		number
	];
}
...
declare function updateFrcCoverageFadeRange(style: Style$1, painter: Painter): void;
declare class HdCoverageState {
	coverageSourceCaches: Record<string, SourceCache>;
	manager: FrcCoverageManager;
	constructor();
}
declare function updateFrcCoverage(style: Style$1, state: HdCoverageState): void;
declare function updateHdCoverageSourceCache(style: Style$1, state: HdCoverageState, layer: TypedStyleLayer): void;
...
declare class LineHDExtension {
	 ...
	 */
	handleFeature(feature: BucketFeature, geometry: Array<Array<Point>>, canonical: CanonicalTileID, elevationFeatures: ElevationFeature[] | undefined, join: string, cap: string, miterLimit: number, roundLimit: number, bucket: LineBucket): boolean;
...
declare class Painter {
	...
	uploadCommonUniforms(context: Context, program: Program$1<ProgramUniformsType[ProgramName]>, tileID?: UnwrappedTileID | null, fogMatrix?: mat4 | null, cutoffParams?: CutoffParams | null): void;
...
type CoreStyleLayer = BackgroundStyleLayer | CircleStyleLayer | CustomStyleLayer | FillExtrusionStyleLayer | FillStyleLayer | HeatmapStyleLayer | HillshadeStyleLayer | LineStyleLayer | RasterStyleLayer | SkyLayer$1 | SymbolStyleLayer;
...
declare function setSdkInfo(info: string): void;
declare class Hash {
