...
import { Sparkles, Send, MessageSquare, X, Loader2 } from 'lucide-react'
...
export function AIAssistant({ suggestions = [] }: AIAssistantProps) {
  ...
  return (
    <div className="fixed inset-0 lg:bottom-8 lg:right-8 lg:w-96 lg:h-[600px] lg:top-auto lg:inset-auto bg-black/50 lg:bg-transparent z-50 flex items-end lg:items-stretch">
      <div className="w-full lg:rounded-2xl bg-white lg:shadow-float flex flex-col h-full lg:h-auto border border-neutral-100/50">
        <div className="bg-primary-500 px-4 py-4 flex items-center justify-between rounded-t-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-primary-50 min-h-0 flex flex-col justify-between">
          {messages.length === 0 ? (
            ...
            <div className="space-y-3">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] px-4 py-2 rounded-2xl text-sm ${msg.type === 'user' ? 'bg-primary-500 text-white rounded-br-none' : 'bg-white text-neutral-700 rounded-bl-none border border-primary-100'}`}>
                    {msg.text}
                  </div>
                </div>
              ))}
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-white px-4 py-3 rounded-2xl rounded-bl-none border border-primary-100">
        ...
        <div className="bg-white border-t border-primary-100 p-3 rounded-b-2xl">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 bg-primary-75 border border-primary-100 rounded-xl text-sm focus:outline-none focus:border-accent-300 focus:ring-1 focus:ring-accent-300 text-black"
            />
            <button
